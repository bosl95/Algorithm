...
### 유의할 점은 스티커를 순서대로 붙일 수 있는 스티커를 먼저 붙여버린다는 것이다. (불필요한 스티커의 회전을 줄여야한다.)<br>
### 시간초과가 났던 나의 풀이와 차이점은 DFS를 이용했고, 스티커를 붙일 수 있는 지 체크하는 과정에서 바로 스티커를 붙인 배열을 새로 return 해주었고 그 return한 배열을 다음 DFS에 사용하는 방식으로 구현했다.<br>
